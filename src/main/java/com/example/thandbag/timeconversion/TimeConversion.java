...
public class TimeConversion {
    ...
    public static String ampmConversion(LocalDateTime createdAt) {
        String resultConversion = createdAt.format(
    ...
    /* 채팅 리스트에서 보여줄 시간 */
    public static String chattingListTimeConversion(
                                            LocalDateTime lastContentTime){
        String resultConversion = "";
        ...
        if (yearToday == yearContent
                && monthToday == monthContent
                && dayToday == dayContent) {
            resultConversion = lastContentTime.format(
                            DateTimeFormatter.ofPattern("a hh:mm")
                            .withLocale(Locale.forLanguageTag("ko")))
            ;
        } else if(yearToday == yearContent
                && monthToday == monthContent
                && (dayToday - dayContent == 1)) {
            resultConversion = "어제";
        } else {
